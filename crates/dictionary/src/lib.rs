...
pub fn encode_dictionary(data: &String) -> String {
    let encoded: Vec<&str> = data.split(",")
        .collect::<Vec<&str>>();
    let n = encoded.len();
...
pub fn decode_dictionary(data: &String) -> String {
    ...
    let mut result = String::with_capacity(data.len() * 100);
    let n = codes.len();
